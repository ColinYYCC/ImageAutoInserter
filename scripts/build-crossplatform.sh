...
case "$OS_TYPE" in
    "Darwin")
        ...
        echo "[步骤3/4] 打包macOS应用..."
        electron-builder --mac

        echo "[步骤4/4] 验证输出文件..."
        if [ -f "release/ImageAutoInserter-1.0.1-arm64.dmg" ]; then
        ...
        echo "[输出] release/ImageAutoInserter-1.0.1-arm64.dmg"
        ;;
    ...
    "CYGWIN"*|"MINGW"*|"MSYS"*|"Windows NT")
        ...
        echo "[步骤4/4] 验证输出文件..."
        if [ -f "release/ImageAutoInserter-1.0.1-setup.exe" ]; then
            echo "[成功] Windows安装包已生成!"
            ls -lh release/*.exe
        elif [ -d "release/win-unpacked" ]; then
