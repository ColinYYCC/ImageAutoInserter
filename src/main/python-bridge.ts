...
import { isWindows, isMac, toLongPath, SYSTEM_CONFIG } from './platform';
...
export class PythonBridge {
  ...
  private getExecutablePath(): string {
    ...
    const scriptPaths = getPythonScriptPath();
    if (isWindows() && scriptPaths.scriptPath.endsWith('.exe')) {
      logInfo(`[PythonBridge] 生产模式：使用 Nuitka 编译 ${scriptPaths.scriptPath}`);
      return scriptPaths.scriptPath;
    }
  ...
  processExcel(
    ...
    if (isWindows()) {
      bridgeEnv.PATH = SYSTEM_CONFIG.process.envPathSetup + path.delimiter + (process.env.PATH || '');
    } else if (isMac()) {
      bridgeEnv.PATH = SYSTEM_CONFIG.process.envPathSetup + path.delimiter + (process.env.PATH || '/usr/bin:/bin:/usr/sbin:/sbin');
    }
    ...
    if (isWindows() && scriptCwd) {
      const longCwd = toLongPath(scriptCwd);
      spawnOptions.cwd = longCwd;
    } else if (scriptCwd) {
