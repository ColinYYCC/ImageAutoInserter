...
class PlatformAdapter {
  ...
  getPlatformInfo(): PlatformInfo {
    ...
    return {
      ...
      envPathSeparator: path.delimiter,
      homeDir: os.homedir(),
      tmpDir: os.tmpdir(),
  ...
  private generateWindowsLibraryPaths(): string[] {
    ...
    for (let drive = 65; drive <= 90; drive++) {
      ...
      const driveRoot = `${driveLetter}:${path.sep}`;

      for (const version of pythonVersions) {
        paths.push(path.join(driveRoot, `Python${version}`));
      }

      paths.push(path.join(driveRoot, 'Anaconda3'));
      paths.push(path.join(driveRoot, 'Miniconda3'));
      paths.push(path.join(driveRoot, 'ProgramData', 'Anaconda3'));
      paths.push(path.join(driveRoot, 'ProgramData', 'Miniconda3'));
    }

    const userHome = os.homedir();
    paths.push(path.join(userHome, 'anaconda3'));
    paths.push(path.join(userHome, 'miniconda3'));
    paths.push(path.join(userHome, '.conda'));
  ...
  private findPythonExecutable(): string {
    ...
    const modernSearchPaths = [
      path.join('C:', 'Program Files', 'Python39', 'python.exe'),
    ...
    const generateAllDrivePaths = (): string[] => {
      ...
      for (let drive = 65; drive <= 90; drive++) {
        ...
        const driveRoot = `${driveLetter}:${path.sep}`;
        for (const version of pythonVersions) {
          drivePaths.push(path.join(driveRoot, `Python${version}`, 'python.exe'));
        }
        drivePaths.push(path.join(driveRoot, 'Anaconda3', 'python.exe'));
        drivePaths.push(path.join(driveRoot, 'Miniconda3', 'python.exe'));
        drivePaths.push(path.join(driveRoot, 'ProgramData', 'Anaconda3', 'python.exe'));
        drivePaths.push(path.join(driveRoot, 'ProgramData', 'Miniconda3', 'python.exe'));
      }
      return drivePaths;
    };
  ...
  toLongPath(p: string): string {
    ...
    if (p.startsWith(`\\\\?\\${path.sep}`) || p.startsWith('\\\\?\\')) return p;
