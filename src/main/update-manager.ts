...
export class UpdateManager {
  ...
  private async handleUpdateError(err: any): Promise<void> {
    ...
    if (errorMessage.includes('429') || errorMessage.includes('Too Many Requests')) {
      logError('❌ 更新请求过于频繁');
      this.state.error = '更新请求过于频繁，请稍后重试';
      this.notifyRenderer('update-error', { error: this.state.error });
      return;
    }
    ...
    if (errorMessage.includes('SSL') || errorMessage.includes('CERT') ||
        errorMessage.includes('certificate') || errorMessage.includes('UNABLE_TO_VERIFY_LEAF_SIGNATURE')) {
      logError('❌ SSL 证书错误');
      this.state.error = '网络连接异常，请检查代理设置或系统时间';
      this.notifyRenderer('update-error', { error: this.state.error });
      return;
    }
    ...
    if (errorMessage.includes('ECONNRESET') || errorMessage.includes('connection reset')) {
      logError('❌ 连接重置');
      this.state.error = '网络连接不稳定，请稍后重试';
      this.notifyRenderer('update-error', { error: this.state.error });
      return;
    }

    if (errorMessage.includes('PROTOCOL') || errorMessage.includes('proxy')) {
      logError('❌ 代理协议错误');
      this.state.error = '网络代理配置异常，请检查代理设置';
      this.notifyRenderer('update-error', { error: this.state.error });
      return;
    }
