...
import { getShortPathName, SYSTEM_CONFIG, isWindows } from '../platform';
import { logWarn, writeLog, log } from '../utils/logging';
...
async function executePythonProcess(
  ...
  const spawnEnv: NodeJS.ProcessEnv = {
    ...process.env,
    PATH: SYSTEM_CONFIG.process.envPathSetup + (isWindows() ? ';' : ':') + (process.env.PATH || ''),
    PYTHONUNBUFFERED: '1',
  ...
  const spawnOptions: {
    ...
    cwd: isWindows() ? getShortPathName(cwd) : cwd,
    ...
    return new Promise((resolve) => {
      ...
      (async () => {
        try {
          ...
          proc.on('close', (code) => {
            writeLog('[IPC] 进程关闭，代码:', code);

            const fullOutput = buffers.stdoutBuffer;
            writeLog('[IPC] ====== DEBUG OUTPUT PATH ======');
            writeLog('[IPC] 完整输出长度:', String(fullOutput.length));
            // 输出完整输出的前1000字符（如果足够长）
            writeLog('[IPC] 完整输出前1000字符:', fullOutput.substring(0, Math.min(1000, fullOutput.length)));
            writeLog('[IPC] 完整输出末尾500字符:', fullOutput.substring(Math.max(0, fullOutput.length - 500)));
            // 统计 ___RESULT_START___ 和 ___RESULT_END___ 出现的次数
            const startCount = (fullOutput.match(/___RESULT_START___/g) || []).length;
            const endCount = (fullOutput.match(/___RESULT_END___/g) || []).length;
            writeLog('[IPC] ___RESULT_START___ 出现次数:', startCount, '___RESULT_END___ 出现次数:', endCount);
            ...
            if (code !== 0 && code !== null) {
              ...
              writeLog('[IPC] 解析失败诊断 - 开始标记:', hasStartMarker, '结束标记:', hasEndMarker);

              // 尝试查找任何 JSON 格式的内容
              const jsonMatch = fullOutput.match(/\{[\s\S]*?\}/);
              if (jsonMatch) {
                writeLog('[IPC] 找到可能的 JSON 内容:', jsonMatch[0].substring(0, 200));
              }
