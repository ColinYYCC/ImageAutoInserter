...
class ExcelProcessor:
    ...
    def __init__(self, file_path: str, read_only: bool = True):
        ...
        self._validate_file()
        self._write_mode = not read_only
    ...
    def _ensure_write_mode(self) -> None:
        ...
        两阶段加载优化：扫描阶段使用 read_only=True 降低内存占用，
        写入阶段按需切换为 read_only=False。切换时使用 data_only=False
        保留原始公式，避免公式被替换为缓存值。
        """
        if self._write_mode:
            return
    ...
    def find_sheet_with_product_code(self) -> Optional[SheetInfo]:
        ...
        for sheet_name in self.workbook.sheetnames:
            ...
            for row_idx, row in enumerate(worksheet.iter_rows(max_row=100), start=1):
                for cell in row:
                    # read_only 模式下 iter_rows 返回 EmptyCell，无 column 属性
                    if not hasattr(cell, 'column'):
                        continue
                    if cell.value and self.PRODUCT_CODE_COLUMN in str(cell.value):
                        header_row = row_idx
                        product_code_column = cell.column
                        header_max_column = max(
                            (c.column for c in row if hasattr(c, 'column') and c.value is not None),
                            default=0
                        )
                        break
    ...
    def add_picture_columns(
        ...
        # 保存操作前已存在的列（不含新添加的列）
        pre_existing_columns = mapper.to_picture_columns()

        # 更新映射器中的列信息（供后续 embed_image 使用）
        for idx, number in enumerate(needed_numbers):
            column_num = start_column + idx
            picture_name = f"{base_word} {number}"
        ...
        return ColumnAdditionResult(
            ...
            existing_columns=pre_existing_columns,
            total_columns=len(pre_existing_columns) + len(added_columns)
        )
