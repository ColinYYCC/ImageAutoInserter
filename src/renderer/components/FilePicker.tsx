...
const FilePicker: React.FC<FilePickerProps> = memo(({
  ...
  const validateFile = async (file: FileInfo) => {
    ...
    try {
      ...
      // 验证图片来源（文件夹、压缩包等）
      const imageSourceTypes = ['folder', 'zip', 'rar', '7z'];
      console.log('[FilePicker] File type:', file.type, 'Path:', file.path);
      console.log('[FilePicker] Is image source?', imageSourceTypes.includes(file.type));
      
      if (imageSourceTypes.includes(file.type)) {
        console.log('[FilePicker] Validating image source...');
        if (typeof window.electronAPI.validateImageSource !== 'function') {
          console.log('[FilePicker] validateImageSource not available');
        ...
        try {
          ...
          console.log('[FilePicker] Image validation result:', imageResult);
          ...
          console.error('[FilePicker] Image validation error:', error);
          // 图片来源验证失败，显示错误
          setValidationError({
        ...
        console.log('[FilePicker] Skipping image source validation');
      }
